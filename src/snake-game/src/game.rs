use heapless::{spsc::Queue, FnvIndexSet};
...
impl Game {
    ...
    /// left of the grid will appear in the rightmost column). Assumes that coordinates
    /// are out of bounds in one dimension only.
...
struct Snake {
    ...
    head: Coords,
    /// Coordinates of the rest of the snake'- body.
    /// The end of the tail is at the front.
    tail: Queue<Coords, 32>,
    /// A set containing all the coordinates that the snake currently occupies
...
impl Snake {
    fn new() -> Self {
        ...
        let initial_tail = Coords { row: 2, col: 2 };
...
/// Define the directions that the snake can move in.
enum Direction {
    Up,
    Down,
    Left,
...
/// The outcome of a single step in the game.
enum StepOutcome {
    ...
    Eat(Coords),
    /// Snake has moved, and nothing else happened.
    Move(Coords),
}

/// A single point in the game grid.
...
impl Coords {
    /// Get random coordinates within a grid.
    /// `exclude` is an optional set of coordinates to exclude.
    fn random(
        rng: &mut Prng,
